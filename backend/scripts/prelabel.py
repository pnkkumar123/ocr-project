...
def prelabel_pdf(pdf_path: Path, out_dir: Path, detector: DoorDetector, max_pages: int | None) -> int:
    data = pdf_path.read_bytes()
    info = ingest_pdf(data, pdf_path.name)
    print(f"\n{pdf_path.name}: {info.page_count} pages ingested")

    # Detect on every page first so we can rank by door count — the pages with
    # the most doors are the floor plans, which is what we want to label.
    per_page = []
...
def main() -> None:
    ...
    detector = DoorDetector(conf=args.conf)
    ...
    for pdf in args.pdfs:
        if not pdf.exists():
            ...
            continue
        grand_total += prelabel_pdf(pdf, out_dir, detector, max_pages)
